...
class AuthResetPasswordController extends Controller
{
    ...
    public function passwordReset(Request $request)
    {
        try {
            ...
            if (!$user) {
                return $this->error('If your email exists in our system, you will receive a password reset link.', 200);
            }
            ...
            // Create reset URL
            $resetUrl = url("/api/v1/password-reset/verify?token=" . $resetToken);
            ...
            ], 'If your email exists in our system, you will receive a password reset link.', 201);
        } catch (\Exception $e) {
            return $this->error('Failed to process password reset: ' . $e->getMessage(), 500);
        }
    }
    ...
    public function passwordResetVerify(Request $request)
    {
        try {
            ...
            return $this->success([
                'user' => $user,
            ], 'Password has been reset successfully.', 201);
        } catch (\Exception $e) {
